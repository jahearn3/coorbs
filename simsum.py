...
def hill_radius(a, m, M=bd.M_Sat()):
...
def compute_n0(a0, masses, M_Sat=bd.M_Sat(), G=cm6.G()):
	M_tot = M_Sat + np.sum(masses) 
	mu = G * M_Sat
...
def summary_plot(bodies, t, a, a0, delta_a, R_H, e, delta_e, lmda, lmda0, delta_lambda, x, y, black=False, units='R_H', t_units='orbits', nrows=60, ncols=27):
	...
	fig, ax1 = plot_semimajor_axis(fig, bodies, a, a0, t, delta_a, R_H, units, t_units, nrows, ncols, 0, 0, 20, 9)
	...
	fig, ax2 = plot_corotating_frame(fig, bodies, a, a0, lmda, lmda0, nrows, ncols, 40, 0, 20, 9)
	N = len(lmda)
	delta_lambda_max = 0
	min_sep_deg = np.zeros(N)
	min_sep_idx = np.zeros(N)
	for i in range(N):
		fig, ax4, diff = plot_phase_space_separated(fig, bodies, i, a, a0, delta_a, lmda, lmda0, delta_lambda, R_H, units, nrows, ncols, i * int(60 / N), 9, int(60 / N), 9)
		if(diff > delta_lambda_max):
			delta_lambda_max = diff
		fig, ax5, min_sep, idx = plot_longitudinal_differences_separated(fig, bodies, i, lmda, t, t_units, nrows, ncols, i * int(60 / N), 18, int(60 / N), 9)
	...
	fig, ax3 = plot_corotating_longitude(fig, bodies, lmda, lmda0, delta_lambda, min_sep_deg, min_sep_idx, t, t_units, nrows, ncols, 20, 0, 20, 9)
	plt.tight_layout()
	ax1.figure.savefig('simulation_summary.png')
	plt.clf()
	return delta_lambda_max, min_sep_deg

def record_data(masses, delta_a, lmda_amp, min_sep_deg, initial_sep_deg, units):
	if(units != 'R_H'):
		units = 'km'
...
def a_plot(bodies, a, a0, t, delta_a, R_H, units='R_H', t_units='orbits', black=False):
	...
	pa.save_and_clear_plot(fig, ax, filename='a_vs_t') 
...
def main(filename='big.in', black=True, units='R_H', t_units='orbits'):
	...
	record_data(masses, delta_a, lmda_amp, min_sep_deg, initial_sep_deg, units)
main(black=False)
